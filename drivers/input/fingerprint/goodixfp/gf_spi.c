...
#define WAKELOCK_HOLD_TIME 1500 /* in ms */

#define GF_SPIDEV_NAME     "goodix,fingerprint"
/*device name after register in charater*/
#define GF_DEV_NAME            "goodix_fp"
#define	GF_INPUT_NAME	    "fpcsensor"	/*"goodix_fp" */

#define	CHRD_DRIVER_NAME	"goodix_fp_spi"
#define	CLASS_NAME		    "goodix_fp"

/* set true in shark factory, default false */
#define POWER_CTL_NODE

#ifdef POWER_CTL_NODE
static bool regulator_flag = false;
#endif

#define N_SPI_MINORS		256	/* ... up to 256 */
static int SPIDEV_MAJOR = 92;
...
static struct regulator *gf_vreg = NULL;
static bool vreg_on = false;


#if 1
#define GOODIX_FPSENSOR_VDD_NAME "vdd_io"
...
static struct pinctrl *fingerprint_pinctrl;
static struct pinctrl_state *pinctrl_state[ARRAY_SIZE(pctl_names)];*/
...
static struct vreg_config const vreg_conf[] = {
	{ "vdd_ana", 1800000UL, 1800000UL, 6000, NULL},
	{ "vcc_spi", 1800000UL, 1800000UL, 10, NULL},
	{ "vdd_io", 3100000UL, 3100000UL, 200000, NULL},
};
#endif
...
#if 1
static int vreg_setup(const char *name, bool enable, struct device *dev)
{
	int i;
	int rc;
	...
	for (i = 0; i < ARRAY_SIZE(vreg_conf); i++) {
		...
		if (!strncmp(n, name, strlen(n))) {
            gf_vreg = vreg_conf[i].vreg;
			goto found;
        }
	}

	pr_err("goodix Regulator %s not found\n", name);

	return -EINVAL;

found:
    if (!gf_vreg) {
        gf_vreg = devm_regulator_get(dev, name);
        printk(KERN_ERR"gf_spi debug get regulator\n");
        if (IS_ERR(gf_vreg)) {
            dev_err(dev, "Unable to get %s\n", name);
            return PTR_ERR(gf_vreg);
        }
    }
	...
	if (enable) {
		...
		if (true == vreg_on) {
            pr_info("goodix already enabled\n");
		    return -EINVAL;
		}

        pr_info("goodix vreg enable\n");

        rc = regulator_set_load(gf_vreg, vreg_conf[i].ua_load);
        if (rc < 0)
            dev_err(dev, "Unable to set current on %s, %d\n",
                    name, rc);

		rc = regulator_set_voltage(gf_vreg, vreg_conf[i].vmin,	vreg_conf[i].vmax);
		if (rc) {
			dev_err(dev,
		...
		rc = regulator_enable(gf_vreg);
		if (rc) {
			dev_err(dev, "error enabling %s: %d\n", name, rc);
			regulator_put(gf_vreg);
			gf_vreg = NULL;
		}
        ...
        vreg_on = true;
			...
			//if (regulator_is_enabled(gf_vreg)) {
			if(true == vreg_on) {
    			pr_info("goodix vreg disable\n");

/*
                rc = regulator_set_voltage(vreg, 0, vreg_conf[i].vmax);
                if (rc) {
					pr_err("goodix disabled set to 0 failed %s\n", name);
                    return rc;
				}
*/
                rc = regulator_set_load(gf_vreg, 0);
                if (rc < 0) {
                    pr_err("goodix Unable to set current on %s, %d\n",
					    name, rc);
                    return rc;
                }
                rc = regulator_disable(gf_vreg);
                if (rc) {
            	...
            	pr_info("goodix vreg already down\n");
            }
		rc = 0;
	}
...
#ifdef POWER_CTL_NODE
static ssize_t fp_power_down(struct device *dev, struct device_attribute *attr, char *buf)
{
    regulator_flag = true;
    pr_info("%s, flag %d\n", __func__, regulator_flag);
    return 0;
}
static DEVICE_ATTR(goodix_power, S_IRUSR, fp_power_down, NULL);

static struct attribute *attributes[] = {
	&dev_attr_goodix_power.attr,
	//&dev_attr_device_prepare.attr,
	NULL
};

static const struct attribute_group fp_attribute_group = {
	.attrs = attributes,
};
...
static int gf_open(struct inode *inode, struct file *filp)
{
    ...
    pr_info("goodix_k open enter 000\n");
    //msleep(5000);
	...
	if (status == 0) {
		...
		pr_info("goodix_k No device for minor %d\n", iminor(inode));
	}
	mutex_unlock(&device_list_lock);
    pr_info("goodix_k open status:%d\n", status);
...
static int gf_suspend(struct platform_device *pdev)
{
    ...
    if (true == regulator_flag ) {
        pr_info("goodix_fp vddio factory power down!\n");
        vreg_setup(GOODIX_FPSENSOR_VDD_NAME, false, &pdev->dev);
    }
...
{
    if (true == regulator_flag ) {
        pr_info("goodix_fp vddio factory power up!\n");
        vreg_setup(GOODIX_FPSENSOR_VDD_NAME, true, &pdev->dev);
    }
...
static int drm_notifier_callback(struct notifier_block *nb, unsigned long val, void *data)
{
	...
	pr_info("[info] %s go to the drm_notifier_callback value = %d\n",
			__func__, (int)val);
	...
	if (evdata && evdata->data && val == MSM_DRM_EVENT_BLANK && gf_dev) {
		...
		case MSM_DRM_BLANK_POWERDOWN:
            pr_info("MSM_DRM_BLANK_POWERDOWN\n");
...
static struct notifier_block fp_noti_block = {
...
#endif
{
    ...
    pr_info("goodix_k minor = %ld\n", minor);
    vreg_setup(GOODIX_FPSENSOR_VDD_NAME, true, &pdev->dev);

    pr_info("goodix_k power on ok\n");
...
#ifdef POWER_CTL_NODE
    status = sysfs_create_group(&pdev->dev.kobj, &fp_attribute_group);
	if (status) {
...
#if defined(CONFIG_DRM)
            gf_dev->notifier = fp_noti_block;
            status = msm_drm_register_client(&gf_dev->notifier);
            if(status) {
                pr_info("register drm_notifier failed. status=%d\n", status);
	...
	pr_info("version V%d.%d.%02d.st:%d\n", VER_MAJOR, VER_MINOR, PATCH_LEVEL, status);
...
static int __init gf_init(void)
{
    ...
    //pr_info("SPIDEV_MAJOR 000 = %d\n", SPIDEV_MAJOR);
	//SPIDEV_MAJOR = status;
    //pr_info("SPIDEV_MAJOR 111 = %d\n", SPIDEV_MAJOR);
